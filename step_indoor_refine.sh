...
srun -p mozi_t --job-name=train_bagel_8gpu --gres=gpu:8 --ntasks-per-node=1 -quotatype=reserved apptainer exec --nv  --bind /mnt:/mnt /mnt/inspurfs/mozi_t/linjingli/apptainer/mllm3r.sif \
    bash -c 'torchrun \
      ...
      --dataset_config_file ./data/configs/step_indoor_refine.yaml \
      --model_path /mnt/inspurfs/mozi_t/linjingli/bagel/BAGEL-7B-MoT \
      ...
      --log_every 1 \
      --results_dir /mnt/inspurfs/mozi_t/linjingli/bagel/models/step_indoor_refine0211 \
      --checkpoint_dir /mnt/petrelfs/linjingli/UMM_Spatial/bagel/results/train_checkpoints/step_indoor_refine0211 \
      --lr 2e-5 \
      --visual_und False \
      --num_workers 1 \
      --save_every 5000 \
